...
const preflight = remote("hostname && id -un && pwd && gh api user --jq .login").split("\n");
if (!["mbp", "mbp.local", "MacBook-Pro.local"].includes(preflight[0] ?? ""))
  throw new Error("Remote hostname mismatch");
if (preflight[1] !== "sean" || preflight[3] !== "allquixotic")
  throw new Error("Unexpected SSH or GitHub identity");
console.log(
  `Verified mbp, SSH user ${preflight[1]}, directory ${preflight[2]}, GitHub account ${preflight[3]}`,
);
...
try {
  ...
  if (repository !== "allquixotic/t3code") throw new Error("GitHub repository mismatch");
