...
To receive passkey approval alerts, open **Settings → General → Notifications**, choose **Notifications only** or **Notifications with sound**, and allow the browser permission prompt. While T3 is open, browser alerts notify for pending SSH, provider, and Remote Environment approvals; the foreground app substitutes its in-app review alert when that setting is enabled. Clicking an alert opens the existing passkey page and does not approve or renew access. Alerts clear when requests finish or expire. There is no push delivery when T3 is closed. Request IDs and deadlines alone are remembered in this browser to avoid repeated alerts after a reload.
...
For Apple Silicon macOS, build the broker and run `node hub/scripts/package-bootstrap.ts /absolute/path/bootstrap.tar darwin` on t3code. Transfer it, the public-key remote configuration, [deploy/org.allquixotic.t3-hub.plist](deploy/org.allquixotic.t3-hub.plist), and `hub/scripts/enroll-macos.sh` through approved broker SSH. Run the installer as administrator with archive/hash, config/hash, plist/hash, and the exact verified remote hostname. It downloads the official Node 26.8.2 interpreter using a pinned SHA256 into its own protected directory; it does not run root code through user-writable Homebrew. There is no remote source build or package-manager installation. It creates the socket group, restart wrapper, launchd service, and `/var/lib/t3-hub/rollback-enrollment.sh`. The hub policy's supervisor path is `/var/lib/t3-hub/connect`. The wrapper discards stale sockets only inside its protected directory, so launchd can recover after a crash or runtime update. Retire any identified unmanaged T3 launch agent separately, retaining its plist and state for rollback.

Headless verification after enrollment (no browser automation): run `node hub/scripts/verify-live.ts request seanaoruslin 300`, approve the exact printed passkey URL for 5 minutes, and run `node hub/scripts/verify-live.ts exercise seanaoruslin revoke`. Repeat with `node hub/scripts/verify-live.ts request seanaoruslin 60`, which defaults the approval page to 1 minute, and run `node hub/scripts/verify-live.ts exercise seanaoruslin expiry`. For another verified identity append `USER HOME HOSTNAME` to `exercise`; for this Mac use `exercise mbp revoke sean /Users/sean MacBook-Pro.local` (or `expiry`). The diagnostic holds pairing credentials only in process memory, checks `hostname`, `id -un`, and `pwd` through the native T3 terminal, then verifies the existing websocket closes and new requests return HTTP 423. It rejects an unexpectedly early disconnection as expiry proof, creates no renewal, and stops if a short expiry approval was not selected.

An open hub browser automatically consumes the remote's one-use startup pairing code. For a concurrent headless test, obtain a separate task-scoped `ssh:ALIAS` approval and set `T3_HUB_VERIFY_SSH_GRANT=REQUEST_ID` when running `exercise`. It checks both approvals, verifies the remote identity, and uses the installed T3 `pair --ttl 1m` command to issue a separate diagnostic code. No credentials are printed or saved, and the environment's deadline is unchanged. This is an administrator verification option; normal remote tools need no grant arguments.
